---
id: zettel.1777272812619
title: "Chain-of-thought memberi struktur reasoning intermediate"
desc: "CoT memberitahu model untuk bekerja step-by-step sebelum menjawab."
updated: 1777272812619
created: 1777272812619
tags:
  - zettel
  - ai
  - the-developers-guide-to-ai
  - chapter-5
---

## Ide inti

CoT memberitahu model untuk bekerja step-by-step sebelum menjawab.

## Quote baseline

> With CoT, we encourage the model to work through the problem step-by-step...

## Kenapa ini core

Ini berguna untuk task multi-step, meski mahal.

## Sumber

- Chapter 5
- [[zettel.literature.the-developers-guide-to-ai]]
- [[book-summaries.the-developers-guide-to-ai]]
